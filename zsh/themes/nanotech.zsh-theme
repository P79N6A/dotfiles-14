  ...
  if [[ "$(command git config --get oh-my-zsh.hide-status 2>/dev/null)" != "1" ]]; then
    ...
    case $STATUS in
      ...
      "CLEAN" )
        ...
        [[ -n $IS_UNPUSHED ]] && GIT_CURRENT_STATUS="${GIT_CURRENT_STATUS}${ZSH_THEME_GIT_PROMPT_CLEAN_UNPUSHED}"
        [[ -n $IS_UNPULLED ]] && GIT_CURRENT_STATUS="${GIT_CURRENT_STATUS}${ZSH_THEME_GIT_PROMPT_CLEAN_UNPULLED}"
        ;;
      "" )
        ;;
    esac
