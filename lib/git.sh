function _git_current_repository() {
...
function _git_prompt_ahead() {
  if [[ -n "$(command git rev-list origin/$(_git_current_branch)..HEAD 2> /dev/null)" ]]; then
    echo "AHEAD"
...
function _git_prompt_behind() {
  if [[ -n "$(command git rev-list HEAD..origin/$(_git_current_branch) 2> /dev/null)" ]]; then
    echo "BEHIND"
...
function _git_prompt_remote() {
  if [[ -n "$(command git show-ref origin/$(_git_current_branch) 2> /dev/null)" ]]; then
    echo "EXISTS"
...
function _prompt_geometry_seconds_to_human_time() {
